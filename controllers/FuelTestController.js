...
module.exports = {
    createFuelTest: async (req, res) => {
        const { EmployeeName, EmployeeId, NozzleId, Product, Opening, Closing, Quantity, TestResult, Date } = req.body;
        try {
            const result = await FuelTesting.create({
                EmployeeName, EmployeeId, NozzleId, Product, Opening, Closing, Quantity, TestResult, Date
            })
            try {
    ...
    deleteFuelTest: async (req, res) => {
        ...
        try {
            ...
            const updatefueltest = await Pump.findByIdAndUpdate(
                PumpId,
                {
                    $pull: {
                        FuelTesting: {
