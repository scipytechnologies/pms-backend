  ...
  try {
    ...
    if (!pump) {
      ...
      const Customer = pump.Customer.find(
        (x) => x.CustomerId == updateData.Customer
      );
      if (!Customer) {
...
module.exports = {
  createCredit: async (req, res) => {
    const { VehicleNumber, Customer, Product, Quantity, Price, Amount } =
      req.body;
    try {

        let serialNumber = 10000;
        try {
          const latestCustomer = await Credit.findOne({
  ...
  updateCredit: async (req, res) => {
    const id = req.params.id;
    const { Token, VehicleNumber, Product, Quantity, Price, Amount, Status } =
      req.body;
    try {
  ...
  deleteCredit: async (req, res) => {
    ...
    try {
      ...
      if (!deletedCredit) {
        return res
          .status(404)
          .json({ error: "Credit you want to delete is not found" });
      }
